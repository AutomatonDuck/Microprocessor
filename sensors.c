//Code modified by Dominic Robbins
...
for (;;)
{
        ...
        {   if(light >= lightroom * 1.03) {P1OUT |=  BIT4; __delay_cycles(200);}    // on if dark
...
// beginning of area for all students changes
        ...
        else
        {
            ...
            if(temp > temproom *1.03) {P1OUT |= BIT5; _delay_cycles(200);}//if temp is higher than 1.03 of temproom, turn LED2 on
            if(temp < temproom *1.01) {P1OUT &= ~BIT5; _delay_cycles(200);} //if temp is lower  than 1.01 of temproom, turn LED2 off
        }
        ...
        if(touch > touchroom * 0.7 && touch < touchroom * 0.9) {}
            ...
            //{P2OUT &= ~BIT0; __delay_cycles(200);}  // off

            else
            {
                if (touch >= touchroom *0.9 && flag == 0 && flag2 == 0)// lift finger
                {
                    flag2 = 1;
                }
                if(touch >= touchroom *0.9 && flag == 1 && flag2 == 0)// on after lifting finger
                {
                    flag2 = 1;
                }
                if(touch <= touchroom*0.7 && flag == 0 && flag2 ==0)
                {

                }
                if(touch <= touchroom*0.7 && flag == 1 && flag2 ==0)// not lifting finger
                {

                }
                if(touch <= touchroom*0.7 && flag == 0 && flag2 ==1) // turn on
                {
                    flag2 = 0;
                    flag = 1;
                    P2OUT |= BIT0;
                }
                if(touch <= touchroom*0.7 && flag == 1 && flag2 ==1)// turn off
                {
                    flag2 = 0;
                    flag = 0;
                    P2OUT &= ~BIT0;
